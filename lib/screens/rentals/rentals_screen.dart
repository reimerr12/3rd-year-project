...
//
// Equipment rental screen — full bilingual support (Bengali / English).
// Google Maps integration: "View on Map" bottom sheet per equipment card.
//
// Prerequisites:
//   1. google_maps_flutter and http in pubspec.yaml
//   2. AndroidManifest.xml:
//      <meta-data android:name="com.google.android.geo.API_KEY"
//                 android:value="YOUR_KEY"/>
//   3. constants.dart has AppConstants.googleMapsApiKey
...
// ---------------------------------------------------------------------------
// Bilingual helper — every widget that needs translation receives [bn].
// ---------------------------------------------------------------------------
...
class _RentalsScreenState extends ConsumerState<RentalsScreen>
  ...
  bool _bn = true; // language toggle — propagated to all children
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _t(_bn, 'সরঞ্জাম ভাড়া', 'Equipment Rental'),
        ...
        bottom: TabBar(
          ...
          tabs: [
            Tab(text: _t(_bn, 'ব্রাউজ করুন', 'Browse')),
            Tab(text: _t(_bn, 'আমার সরঞ্জাম', 'My Equipment')),
          ],
        ),
      ),
      body: TabBarView(
    ...
    return showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(bn
            ...
            : '${equipment.name} will be permanently removed from your listings.'),
        actions: [
          TextButton(
      ...
      data: (bookings) {
        if (bookings.isEmpty) {
          return _EmptyView(
            ...
            message: _t(bn, 'আপনার সরঞ্জামে কোনো বুকিং নেই', 'No bookings yet'),
          );
        }
        return RefreshIndicator(
          color: AppTheme.primaryGreen,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(children: [
            Text(
              _currFmt.format(booking.totalCost),
              style: const TextStyle(
                fontSize: 14,
                fontWeight: FontWeight.w700,
                color: AppTheme.primaryGreen,
          ...
          if (status == BookingStatus.active) ...[
            ...
            _ActionButton(
              ...
              onTap: () => notifier.updateStatus(
                  bookingId: booking.id, status: BookingStatus.completed.value),
            ),
          ],
        ],
      ),
...
class _AddEquipmentSheetState extends ConsumerState<_AddEquipmentSheet> {
  ...
  LatLng? _pickedLatLng; // confirmed pin position
  bool _locationSearching = false; // geocoding in progress
  bool _locationMapVisible = false; // show the embedded map preview
  ...
  // Geocode the typed location text and show map preview
  Future<void> _searchLocation() async {
    ...
    try {
      // Build a Bangladesh-biased query
      final fullQuery =
      ...
      if (coords != null) {
        ...
        _locationMapCtrl?.animateCamera(
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(_t(
              bn,
              'অবস্থান খুঁজে পাওয়া যায়নি। অন্য নাম দিয়ে চেষ্টা করুন।',
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              _t(bn, 'নতুন সরঞ্জাম যোগ করুন', 'Add New Equipment'),
              style: const TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.w700,
                color: AppTheme.darkGreen,
              ...
              children: EquipmentType.values.map((t) {
                ...
                return ChoiceChip(
                  ...
                  labelStyle: TextStyle(
                    color:
                        selected ? AppTheme.darkGreen : AppTheme.textSecondary,
                    fontWeight: selected ? FontWeight.w700 : FontWeight.normal,
                  ),
                  side: BorderSide(
                    color:
                        selected ? AppTheme.primaryGreen : AppTheme.borderGrey,
                  ),
                );
              }).toList(),
            ),
            ...
            Row(children: [
              Text(
                _t(bn, 'সর্বনিম্ন বুকিং দিন', 'Minimum Booking Days'),
                style: const TextStyle(
            ...
            Container(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  hint: Text(
                    _t(bn, 'বিভাগ (ঐচ্ছিক)', 'Division (optional)'),
                    style: const TextStyle(
              ...
              Padding(
                ...
                child: Text(
                  _t(
                      ...
                      'Tap on map or drag the pin to set the exact location'),
                  style: const TextStyle(
                      fontSize: 11, color: AppTheme.textSecondary),
                ),
              ),
            ...
            // Image picker
            Text(
              _t(bn, 'ছবি যোগ করুন (ঐচ্ছিক)', 'Add Photos (optional)'),
              style: const TextStyle(
                  fontSize: 13,
                  color: AppTheme.textSecondary,
                  fontWeight: FontWeight.w500),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isLoading
                    ...
                    : Text(
                        _t(bn, 'সরঞ্জাম যোগ করুন', 'Add Equipment'),
                        style: const TextStyle(
                            fontSize: 16, fontWeight: FontWeight.w700),
                      ),
              ),
...
class _StatusChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        bn ? status.labelBn : status.labelEn,
        style:
...
class _ErrorView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Detect language from message to label the retry button
